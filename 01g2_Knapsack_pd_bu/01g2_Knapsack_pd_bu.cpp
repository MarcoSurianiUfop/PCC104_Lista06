/******************************************************************************
                 ...
                 LISTA 06 - ATIVIDADES PRÁTICAS - EXERCÍCIO 01G
                      KNAPSACK PROG DINAMICA - BOTTOM-UP
...
int BUKnapSack(vector<vector<int>>& I, vector<vector<int>>& F, int n, int W) {
    /*  Implementação do método da função de memória para o KNAPSACK PROBLEM
    * Resolução via Prog Dinamica - BOTTOM-UP
    * ENTRADAS:
    ...
    */
    ...
    // Itera por todas posições [1..n][1..W] de F
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= W; j++) {
            v1 = BUKnapSack(I, F, i - 1, j);
            if (j < I[0][i - 1]) { F[i][j] = v1; }
            else {
                F[i][j] = max(v1, I[1][i - 1] + BUKnapSack(I, F, i - 1, j - I[0][i - 1]));
            }
        }
    }
    return F[n][W];
...
int KnapSackProblem_pd_bu(vector<vector<int>>& I, int& W) {
    /*  Implementação do KNAPSACK PROBLEM via Prog Dinamica - BOTTOM-UP
...
int main() {
    ...
    int max_knap = KnapSackProblem_pd_bu(V, W);
